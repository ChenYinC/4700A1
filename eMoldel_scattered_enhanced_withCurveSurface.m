...
%inserted circles by set the centre (cx, cy) and radius: [cx, cy, r]
circ_num = 2; %number of circle
circ = [8 2 1; 8 8 1];
ang = 0:0.01:2*pi
for c=1:circ_num
    xp = circ(c, 3) * cos(ang);
...
for i=1:iter_num
    ...
    for j=1:e_num
        ...
        %check inserted circles
        for c=1: circ_num
            
            x_dist = e_posx(j) - circ(c, 1);
            y_dist = e_posy(j) - circ(c, 2);
            dist = sqrt(x_dist^2 + y_dist^2); %determin the distance between electron and circle center
            ang_circ = atand(x_dist / y_dist); %determin the ang of electron to the center of circle

            if (dist <= 1.1*circ(c, 3)) %if the electron position hit the circle 
                temp = e_posy_old(j) - e_posy(j);
                temp2 = e_posx_old(j) - e_posx(j);
                ang_in = atand(temp / temp2); %entrane angle (xy coordinate)
                phi2 = 2*ang_circ + 2*ang_in - 360;
                ang_out = ang_in - phi2;

                v_circ = sqrt(e_vx(j)^2 + e_vy(j)^2);
                e_vx(j) =  -v_circ * cosd(ang_out); %new Vx
                e_vy(j) =  -v_circ * sind(ang_out); %new Vy             
            end        
        end
